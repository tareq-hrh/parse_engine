...
import { serializeThink } from "@/lib/prismaHelpers";
import {
...
export async function GET() {
  try {
    const extractionJobs = await getExtractionJobSnapshots();
    return NextResponse.json(extractionJobs, { status: 200 });
  } catch (error) {
...
export async function POST(req: NextRequest) {
  try {
    ...
    const createdSnapshot = await getExtractionJobSnapshot(created.id);
    if (!createdSnapshot) {
      return NextResponse.json(
        { error: "Created extraction job could not be loaded." },
        { status: 500 },
      );
    }

    return NextResponse.json(createdSnapshot, { status: 201 });
  } catch (error) {
